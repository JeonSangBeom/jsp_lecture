...
<script>
	...
	$("#summernote").summernote({
		height : 300,
		callbacks : { // 답신하겠단 뜻
			onImageUpload : function(files) { // onImageUpload - 이미지 업로드시 작동
				/* uploadImage(files[0],this); - 하나씩 올릴 수 있는 방법 */
				for(let i=0;i<files.length;i++){
					uploadImage(files[i],this);	// files가 배열로 여러개가 올수 있어서 이런식으로 설정(this는 나 자신에게 보내겠다는 뜻)
				}// 여러개 첨부하여 사용할 경우(한번에)
				// 이미지 업로드  
			}
		}
	...
	// 데이터 전달할때 사용한 것들
	//1) queryString  ===>       view.do?img=aaa&text=bbb   (get 방식)
	//2) form 태그를 이용하는 방법   <form method="GET 또는 POST" action="넘길 페이지"></form>
	//3) FormData(form과 비슷) - 아래 방식(ajax)
	function uploadImage(file,editor) { // uploadImage함수 생성
		const sendData = new FormData(); // 넘길 데이터는 FormData(작성할 데이터란 뜻) 생성
		sendData.append("uploadFile",file); // 작성할 데이터에 넣어주기 "uploadFile" - 이건 아무거나 써도 상관 없다(키와 같은 의미) / file - 위에서 매개변수로 넘기고 있는 것
				//<input type="file" name"uploadFile"> 과 같은 것
		$.ajax({
			url:"SummerNoteFileUpload.do", // 보낼 곳
			type:"POST", // 안쓰면 디폴트가 깨진다 / 메소드 폼에서 쓰는 것과 동일하게 써주면 된다(멀티파트는 post만 가능)
			data:sendData, // FormDate로 만든 sendData
			contentType:false,// 꼭 써야 하는 것 - ajax에서
			// 안쓰게 되면 이렇게 application/x-www-form-urlencode(디폴트값)로 넘어간다(form데이터로 넘어가지 않는다)
			// false로 해야지만 multipart/form-data로 날아간다
			processData:false,// 꼭 써야 하는 것(안쓰면 쿼리스트링 형식으로 날아간다)
