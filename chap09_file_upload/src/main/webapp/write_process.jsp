...
<%
	...
	String savePath = "upload_mario";// 먼저 디렉토리 하나 만들고 그것과 같은 이름으로 변수 설정(이미지가 올라갈 경로)
	ServletContext context =  this.getServletContext();  // ServletContext - 현재 돌아가는(톰캣 실행중인 것) context(경로-this를 사용하였기 때문에 - chap09_file_upload)를 얻어 오는 것
	String realPath = context.getRealPath(savePath); //현재 경로에 있는 진짜 경로의 값을 알려준다(savePath-변수로 미리 설정해둔 값)
	int fileSize = 1024*1024*100;//100메가바이트
	...
	//cos.jar 안에 있는 것  (request(기본),저장되는 경로,파일 사이즈 , encding (utf-8) , fileRenamePolicy(파일명이 같으면 어떻게 넣을지) )
	//여기까지가 파일 업로드 되는 것
	...
	if(result>0) {
		ScriptWriter.alertAndNext(response, "이미지 업로드가 되었습니다.", "/chap09_file_upload");
	} else {
