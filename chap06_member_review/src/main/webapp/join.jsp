...
<%@ include file="./include/header.jsp"  %>
    ...
    <script>
		...
		// 쿼리 스트링 - <a href="join_process.jsp?user_id=hulk">보내기</a> - get방식
		// ajax로 데이터 보낼 수 있다.
    
    	let idCheck = false; // 아이디 중복 체크 여부 설정 시 미리 지정
    	const korean = /[ㄱ-ㅎㅏ-ㅣ가-힣]/g;
    	const email = /^[0-9a-zA-Z]([-_\.]?[0-9a-zA-Z])*@[0-9a-zA-Z]([-_\.]?[0-9a-zA-Z])*\.[a-zA-Z]{2,3}$/;//이메일 정규식(정규표현식)
    	...
    	$(".btnIdCheck").on("click",function(e){ 
    		...
    		const user_id = $("#user_id").val(); // user_id라는 변수에 #user_id(#=id) 값을 넣어둔다
    		if(user_id==="") { // 만약 공백이라면
    			alert("아이디를 입력해 주세요.");
    		...
    		$.ajax({
    			...
    			type:"POST",// 타입(메서드 get과 post / 둘다 가능)
    			data:{"user_id":user_id},// 실어 보낼 것 - 오브젝트 형식 / 위에 변수로 입력된 값 
    			success:function(res){// 통신이 성공했을때 받을 정보
    				console.log(res);
    				if(res.count > 0) { // res.count : 넘어온 값
    					...
    					const yes = confirm("사용가능한 아이디입니다. 사용하시겠어요");//confirm : 확인, 취소창이 뜨게 할수 있다(true false)
    					if(yes) { //yes일 때 할 일
    	...
    	$(".btnConfirm").on("click",function(){
    		if($("#user_id").val()=="") {
    			...
    			$("#user_email").focus();
    			return false;
    		} else if(!email.test($("#user_email").val())) { //email(변수 지정된 것).test라는 정규표현식 메서드를 사용하여 값과 비교하는 것
